var user = {
    ...
    initEvent: function () {
        ...
        $("input[name='username']").bind("blur", function () {
            ...
            if (filter.test(username)) {
                ...
                $("#span1").text(userFormError).css("color", "red");
            }
        });
    },
    checkUser: function (username) {
        ...
        $.ajax({
            type: "POST",
            url: 'preuserJSONAction_findByName.action',
            data: parameter,
            success: function (data) {
                //alert(data);
                if (data.message == nameSuccess) {
                    $("#span1").text(data.message).css("color", correctColor);
                } else {
                    $("#span1").text(data.message).css("color", errorColor);
                }
            },
            error: function (data) {
                $("#span1").text(unKnowError).css("color", errorColor);
            }
        });
    },
    checkMail: function () {
